...
def hrvElaboration(videofilepath,showImages=False, showVideo=False, showArousal=False):
	...
	if (cap.isOpened() == False): 
		print("[info hrvElaboration] Unable to read the video")

	print("[info hrvElaboration] Processing the video with the hrv mode...")
	 # Find OpenCV version
	...
	print("[info hrvElaboration] The fps of the video is: "+str(fps)+" , "+ "and the number of frames is: "+str(videoLen) )
	print("[info hrvElaboration] Please wait while it is calculating:")
		...
		if(frame is not None):
			try:
				...
				if len(faces) == 1: 		#if the detection is of just 1 face 
					...
					if len(faces) == 0:
						print("[info hrvElaboration] no face detected in a frame")
					else:
						print("[info hrvElaboration] too many faces detected in a frame")
				...
				if (currentFrame>0 and currentFrame%fps==0) :
					#try:
					bpm, sf = functions.hrElaboration(face_red_means, face_green_means, face_blue_means, None, fps, False ,False, False, currentFrame,showArousal=showArousal)
			...
			print("[exception hrvElaboration] Null frame at frame: "+ str(currentFrame)+" of "+str(videoLen) +".")
			break # just for now
