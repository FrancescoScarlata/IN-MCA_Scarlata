...
class GreenWebcamThread:
	...
	def greenElaboration(self):
		...
		if time.clock() > 3:    		# skip the first seconds to setup the focus of the camera
			if 0 <= self.max_samples-len(self.means) and self.remaining!=-1:
				cv2.putText(self.frame, str(self.remaining), (self.leftOffset, self.heightOffsetSeconds), 
				cv2.FONT_HERSHEY_SIMPLEX, self.tsize, self.redCol) 		# the countdown displayed
