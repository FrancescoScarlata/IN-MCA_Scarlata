...
def dynPcaElaboration(filepath, showVideo=False):
	...
	secondOfFrame=10				# the window of seconds on which clear the means saved. Default 10 secs
	...
	print("[info dynPcaElaboration] Processing the video with the dynamic pca mode...")
	...
	for currentFrame in range(0, videoLen):
		...
		try: 
			...
			print("[exception dynPcaElaboration] At frame: "+ str(currentFrame)+" of "+str(videoLen) +" there is no frame. Terminating...")
			#print("[exception dynPcaElaboration] cap.opened: "+str(cap.isOpened()))
			break	#just for now
	print()
	cap.release()
	return bpms
