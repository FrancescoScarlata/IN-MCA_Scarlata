...
class hrFromDynGreen (hrFromX):
	'''
		Class that determines the hr with the green dynamic method
	'''
	def __init__(self,videoPath):
		super().__init__(videoPath, "hr dynGreen")	
	
	def importMethodScript(self):
		''' The method needed'''
		import ArrotaVideoScripts.approccio1Dyn as green
		self.green=green

	def determineHeartRate(self, showVideo=False):
		''' Determine the health rate series and return it'''
		self.heartrate=self.green.dynGreenElaboration(os.path.join(self.videoPath,self.videoName),showVideo)
...
if __name__=='__main__':
	...
	# check if the video input is correct
		...
		if(hr.existsFile()):
			...
			if(heartrate[-1][1]<0):
				print("the hearthrate is certainly incorrect. There was an exception") 
