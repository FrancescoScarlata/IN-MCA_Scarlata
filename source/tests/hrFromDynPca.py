...
'''
class hrFromDynPca (hrFromX):
	'''
		Class that determines the hr with the pca method
	'''
	def __init__(self,videoPath):
		super().__init__(videoPath, "hr dynPca")	
		
	def importMethodScript(self):
		''' The method needed'''
		import ArrotaVideoScripts.approccio3Dyn as pca
		self.pca=pca

	def determineHeartRate(self, showVideo=False):
		''' Determine the health rate series and return it'''
		self.heartrate=self.pca.dynPcaElaboration(os.path.join(self.videoPath,self.videoName),showVideo)
...
if __name__=='__main__':
	...
	if args["video"] is not None:
		...
		hr=hrFromDynPca(str(args["video"]))

		if(hr.existsFile()):
			#print("[info hrFromDynPca] File exists")
			# now that the file exists we can check for the file
			hr.determineHeartRate(True)
			heartrate= hr.getHeartRate()
			if(heartrate[-1][1]<0):
				print("[info "+os.path.basename(__file__)[:-3]+"] the heartrate is certainly incorrect.") 	
			if(heartrate[-1][1]>0):
				print("[info "+os.path.basename(__file__)[:-3]+"] The hearth rate should be "+ str(heartrate[-1][1]))
				print(heartrate)				
		else:	
